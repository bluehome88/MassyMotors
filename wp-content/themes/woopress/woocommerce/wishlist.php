...
<form id="yith-wcwl-form" action="<?php echo esc_url( $yith_wcwl->get_wishlist_url() ) ?>" method="post">
    ...
    ?>
    <table class="shop_table table table-bordered cart wishlist_table <?php if( count( $wishlist ) == 0 ) echo 'empty-table' ?>" cellspacing="0">
        <thead>
        <tr>
            ...
            <?php if( get_option( 'yith_wcwl_stock_show' ) == 'yes' ) : ?><th class="product-stock-status"><span class="nobr"><?php _e( 'Stock Status', 'yit' ) ?></span></th><?php endif ?>
