...
class PMWI_Import_Record extends PMWI_Model_Record {		
	...
	public function import( $importData = array() ){
		...
		if ( $product_type == 'variable' and ! $this->options['link_all_variations'] and "xml" != $import->options['matching_parent'] ){									
			...
			if ( $is_variation_attributes_defined or $this->options['duplicate_matching'] == 'manual'){
				...
				if ( ! empty($product_parent_post_id) and ($product_parent_post_id != $pid or ($product_parent_post_id == $pid and $first_is_parent == "no")) ) {		
					...
					if ( $create_new_variation and ! empty($this->previousID) and $this->previousID != $product_parent_post_id)
						$set_defaults = true;
					else
			...
			// when this is variable product ( not variation )	
			if ( $set_defaults or empty($product_types[$i+1]) ) {

				if ( ! empty($this->previousID) )
				{

					$parent_products = array($this->previousID);					

					if (empty($product_types[$i+1])) $parent_products[] = $product_parent_post_id;
		...
		if ( "variable" == $product_type and "xml" == $import->options['matching_parent'] and "" != $import->options['variations_xpath'] and "" != $import->options['variable_sku'] and ! $import->options['link_all_variations']) {
			...
			if ( $count_variations > 0 ){				
				...
				foreach ($variation_sku as $j => $void) {	if ("" == $variation_sku[$j]) continue;
					...
					// Save shipping class					
					$variation_product_shipping_class[ $j ] = ! empty( $variation_product_shipping_class[ $j ] ) ? (int) $variation_product_shipping_class[ $j ] : '';
					...
					// Manage stock
					/*if (empty($articleData['ID']) or $this->is_update_custom_field($existing_variation_meta_keys, $import->options, '_manage_stock')) 
						update_post_meta( $variation_to_update_id, '_manage_stock', $variation_product_manage_stock[$j] );	
					if (empty($articleData['ID']) or $this->is_update_custom_field($existing_variation_meta_keys, $import->options, '_stock')) 
						update_post_meta( $variation_to_update_id, '_stock', (int) $variation_stock[$j] );											

					if ( $variation_product_manage_stock[$j] == 'yes' ){
						if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_stock_status')){ 
							$v_stock_status = 'instock';
							if ($import->options['product_stock_status'] == 'auto') {
								$v_stock_status = ($variation_stock[$j] === 0) ? 'outofstock' : 'instock';
							}
							update_post_meta( $variation_to_update_id, '_stock_status', $v_stock_status);
						}
					}
					else{
						if (empty($articleData['ID']) or $this->is_update_custom_field($existing_meta_keys, $import->options, '_stock_status')){ 
							$v_stock_status = 'outofstock';
							if ($import->options['product_stock_status'] == 'auto') {
								$v_stock_status = ($variation_stock[$j] === 0) ? 'outofstock' : 'instock';
							}
							update_post_meta( $variation_to_update_id, '_stock_status', $v_stock_status );
