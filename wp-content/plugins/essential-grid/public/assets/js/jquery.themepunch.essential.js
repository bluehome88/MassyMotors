...
function addAjaxNavigagtion(opt,act) {
	...
	act.find('.eg-ajax-right').click(function() {
		...
		if (nexts.length>0) {
			nexts[0].trigger("click");
		} else {
			prevs[0].trigger("click");
		}
	});
	...
	act.find('.eg-ajax-left').click(function() {
		...
		if (prevs.length>0) {
			prevs[prevs.length-1].trigger("click");
		} else {
			nexts[nexts.length-1].trigger("click");
		}
	});
}
			 ...
			 if (clickable=="on" && eec.find('.esg-overlay').length>=1) {
				 eec.click(function(e) {
					if (jQuery(e.target).closest('a').length==0)
					  jQuery(this).find('.eg-invisiblebutton')[0].trigger("click");
				 }).css({cursor:"pointer"});
			 }
		 })
